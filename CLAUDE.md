...
- Type Sync: `tauri-specta`；Rust struct / enum 是跨端数据模型来源。
- Runtime: Node.js 20+、pnpm、Rust stable。

## 3. 核心目录结构

项目结构按 `docs/DESIGN.md` 规划组织；实现时只创建当前功能实际需要的文件，不为未来功能创建未接入的占位文件。
...
- 未经确认，不修改 `CLAUDE.md`、`README.md`、`.gitignore`、核心配置或 `docs/DESIGN.md`。
- 禁止私自新增第三方依赖。
- 禁止为了代码风格重写稳定代码。
- 禁止删除有效业务逻辑和关键注释。
- 禁止实现超出 v0.1 的“顺手功能”。
...
- [ ] 是否符合 `docs/DESIGN.md`，尤其 v0.1 范围和架构红线。
- [ ] 是否只修改任务相关内容。
