...
## Phase 2：多目标真实跑通
- [ ] **T32** 多目标 plan/execute（`targets[]`，operations 关联 target）+ target-aware ExportConflict（四维）+ Cursor 第二适配器 + CC+Cursor headless e2e（建议 2 个 checkpoint commit）— 依赖：T31 — M–L
