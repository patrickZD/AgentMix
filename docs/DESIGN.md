...
本文档描述 AgentMix 从 v0.1 到 v1.0+ 的产品规划。**v0.1 Alpha 仅交付以下 11 项功能**，其他功能按 §12 路线图分布在后续版本。评估 v0.1 范围时以本节为准。

1. Tauri + React + TypeScript 应用框架
2. 欢迎屏（空状态） + 主界面两栏布局 + Toolbar
3. 本地目录拖入 + 递归 SKILL.md 扫描 + 三分类结果（portable / tool-specific / invalid）
4. 复选框选择 + ExportConflict 检测 + "重命名 / 保留一个"冲突解决
5. Dry-run 两阶段提交（preview → confirmation → execution）+ 基础备份到 `~/.agentmix/backups/`
6. 单目标导出：**Claude Code 项目级**
7. `scripts/` 安全预检（含 §6.11 完整威胁模型）
8. 健康检查**确定性**部分（frontmatter 合规、name 一致、触发场景动词存在、脚本依赖）
9. i18n 架构（`react-i18next` + `t()` 红线，`en.json` 完整、`zh.json` 关键 key stub）
10. Asset 抽象 + ExportPlan 数据模型作为架构红线落地
11. 静态发布到 GitHub Releases。**v0.1 无自动更新机制**——alpha 阶段用户预期手动升级

明确**不**在 v0.1：手动合并工作台（v0.1.5）、自动更新（v0.1.5）、i18n 翻译完整化（v0.1.5）、多目标导出 / AI 全家桶 / OS keychain 密钥 / `.agentmix.lock` / Source Tracking / Git URL 导入 / 颜色聚类 / Skill 脚手架 / 跨工具兼容性预检 / 引用关系检测 / Skill 编辑器 / 安全规则误报白名单（均 v0.2）、反向同步 / 非技术用户简化模式 / 本地 embedding（均 v0.3）、Asset 类型扩展（v1.0+）。
...
7. [交互设计](#7-交互设计)
8. [技术架构](#8-技术架构)
9. [核心技术挑战与解决方案](#9-核心技术挑战与解决方案)
10. [验收标准与测试策略](#10-验收标准与测试策略)
11. [开源策略](#11-开源策略)
12. [路线图](#12-路线图)
13. [附录:关键设计决策记录](#13-附录关键设计决策记录)
14. [参考资料](#14-参考资料)
...
**痛点 4：非技术用户完全无法操作。** 现有工具（skills.sh CLI 等）均为命令行工具，对非技术用户极不友好，将大量潜在用户排除在外。
...
**v0.3+ 引入非技术用户简化模式**：产品经理、设计师等使用 AI 编程工具但不熟悉命令行的用户，希望复用他人分享的 Skills 配置。这类用户的体验目标（隐藏路径、术语翻译、一键应用到检测到的工具）需要专门的 UI 层实现——在 v0.1–v0.2 完整闭环验证后再投入，避免一开始就分裂开发节奏。
...
**场景 C：非技术用户复用（v0.3+ 场景）。** 非技术用户从同事或社区获得了一个项目文件夹，希望将其中的 Skills 应用到自己的 AI 工具中，但不想接触命令行或理解 YAML frontmatter 格式。
...
通过对现有工具的系统分析，可以发现以下四个尚未被满足的需求：

第一，**跨项目可视化组合**：没有任何工具支持从多个不同项目中可视化地挑选 Skills 并组合。第二，**冲突实时检测**：现有工具均不提供 Skills 名称冲突的实时预警，冲突只在 Agent 运行时才暴露。第三，**多目标一键导出**：没有工具能将组合结果同时部署到多个 Agent 工具的对应路径。第四，**非技术用户友好界面**：所有现有工具均为 CLI，将非技术用户完全排除在外。
...
| **tool-specific** | spec 合规 + 使用了实验性 / 工具特定字段 | 默认展示，附"仅 X 工具支持"徽章；导出时由兼容性预检（§6.10）拦截 |
| **invalid** | 缺 `name` 或 `description`、name 与父目录名不一致、YAML 解析失败 | **默认隐藏**，设置中可切换"显示无效候选"。提供"一键补全 name"修复入口跳转 §6.6 编辑器 |
...
| 检测时机 | 选中即检测，导出前再次校验 | 导出前由 §6.10 兼容性预检子模块计算 |
...
### 6.3 Skill 合并工作台（Merge Workbench）
...
>
> **路径权威性**：以上数据并非通过实时探测各工具版本得出，而是由社区维护的 `tool-adapters.json` 文件（§6.10 同源）随 AgentMix 版本发布并支持远程更新。UI 角落显示数据新鲜度，超过 14 天提示用户检查。
...
>
> **预览是必经步骤**：导出按钮触发的是预览页 (Dry-run，§6.12)，用户确认后才真正修改文件。除了导出预览这条路径，没有其他修改用户文件的代码分支。

### 6.5 Skill 健康度检查（Health Check）
...
**`scripts/` 行为分析**：在 `scripts/` 静态安全规则（§6.11）之上，调用 LLM 总结脚本意图，与 Skill description 声明的能力做交叉验证。若 LLM 判断"脚本行为与 description 不一致"则标红。
...
| 对象 | 形态 | 职责 |
|---|---|---|
| **工作集 (Composition)** | 内存运行时对象（§8.2） | 用户当前正在编辑的工作集：选中、重命名、合并草稿、目标工具偏好 |
| **Preset / Profile** | 持久化快照（YAML / Bundle zip） | 工作集的可保存版本，含合并指令、目标偏好等 AgentMix 自有信息 |
| **`.agentmix.lock`** | 复现规约（YAML，§6.9） | 仅描述"每个 Skill 的来源 + 版本指针 + 完整性 hash"，schema 严格收敛到 7 字段 |

**关键边界**：Preset 与 lock **不共用一个文件**。lock 的字段集是开放共享契约（决策 23），不能被 AgentMix 私有字段污染；Preset 是 AgentMix 自身的产品形态，可自由演进。
...
### 6.8 来源项目订阅与更新提醒（Source Tracking）
...
### 6.10 跨工具兼容性预检（Capability Linter）
...
### 6.13 Skill 脚手架（New Skill Wizard）
...
5. **同一套校验**：向导内的 description 实时校验复用 §6.5 的结构化检查（触发动词存在、长度上限），保证新建与导入的 Skills 走同一信任路径。
...
脚手架产物是一个合法的 Skill 目录，立即被 AgentMix 自身扫描器识别，与从 GitHub 导入的 Skill 一致。与 §6.3 合并工作台共享同一套"目录构建器"组件。
...
**降级条件**：API key 未配置或 embedding API 调用失败 → 该模式不可用，按钮灰显并提示原因（与 §6.5 AI 增强检查同样的降级策略）。
...
提供"简单模式"切换，隐藏技术细节（文件路径、frontmatter 字段名等），使用更友好的描述（如"代码审查技能"而非 `code-review-and-quality`），导出时自动检测已安装的 AI 工具并推荐导出目标，并提供一键"应用到所有已检测工具"选项。
...
>
> 这个抽象层在 v0.1 必须立起来，v1.0+ 才能低代价扩展到 Slash Command、Subagent、CLAUDE.md 片段、MCP 配置等其他 Agent 资产（见 §12 Phase 4）。
...
### 9.3 挑战：description 质量影响 Skill 触发率
...
**解决方案**：v0.1 起就引入 `Asset` 抽象（§8.2），所有扫描、组合、冲突检测、导出代码通过 `Asset` 接口工作，不直接耦合 `Skill` 具体类型。`Skill` 是 v0.1 唯一实现，但抽象层必须存在。Code review 阶段重点检查"是否有 `as Skill` 或 `kind === 'skill'` 的硬编码分支"，发现即拒。

### 9.7 挑战：AI 合并所需的 API 密钥管理
...
- **导入项目按钮入口**：拖拽与"选择文件夹"按钮等价提供（§6.1），e2e 通过按钮入口完成导入。
- **dialog plugin 测试模式**：`tauri-plugin-dialog` 在测试构建产物中读取预置路径，由构建期 feature flag 控制（§8.1）。
...
- **ExportPlan 一致性测**：同一 Composition 生成的 ExportPlan，Dry-run 渲染的 FileOperation 列表与 `ExportCoordinator.execute` 实际产生的文件结果（路径、数量、字节数）必须完全相同。这条对应 DoD-3，并兜底 §6.12 与决策 21。
- **Asset 抽象纯度测**：pipeline 代码（scanner / composer / exporter 等）grep `instanceof Skill` / `kind === 'skill'` / `as Skill` 命中数 = 0。对应 §9.6、决策 9。
- **名称大小写不敏感冲突测**：`Code-Review` 与 `code-review` 必须被识别为同一冲突。对应 §9.8。
- **备份位置隔离测**：执行任意导出后，断言 `.agentmix-backup-*.zip` 仅出现在 `~/.agentmix/backups/<project-hash>/`，目标项目目录树内零出现。对应 DoD-8、决策 11。
...
v0.1 范围（与文档头 v0.1 Alpha 版本范围一致）：

- Tauri + React + TypeScript 应用框架
- 欢迎屏（空状态） + 主界面两栏布局 + Toolbar（§7.1）
- 本地目录拖入 + 递归 SKILL.md 扫描 + 三分类（portable / tool-specific / invalid，§6.1）
- 复选框选择 + **ExportConflict 检测** + "重命名 / 保留一个"冲突解决（§6.2）
- Dry-run 两阶段提交：preview → confirmation → execution，**消费同一 `ExportPlan` 对象**（§6.12、§8.2）
- 基础备份到 `~/.agentmix/backups/<project-hash>/`（§6.4）
- 单目标导出：**Claude Code 项目级**（仅 `.claude/skills/`）
- `scripts/` 安全预检（完整威胁模型：symlink 不跟随 / 2MB 体积上限 / 二进制资产清单 / 高危脚本规则，§6.11）
- 健康检查确定性部分（frontmatter 合规、name 一致、触发动词存在、脚本依赖，§6.5）
- **i18n 架构红线**：`react-i18next` + `t()` lookup 全覆盖；`en.json` 完整 + `zh.json` 关键 key stub（§6.17）
- **Asset 抽象 + ExportPlan 数据模型作为架构红线**落地（§8.2、§9.6）
...
- **自动更新机制**：GitHub Releases + Tauri Updater，签名密钥就位（§6.16）
- **手动合并工作台**（3 列布局 + 实时校验 + frontmatter 重组，§6.3）
...
- 多目标导出：Cursor / Codex CLI / OpenCode / Gemini CLI（按 ToolAdapter 配置，§6.4）
- 全局路径导出（作用范围选择「项目级 / 全局」，选「全局」时路径由 AgentMix 自动解析为 `~/.claude/skills/`，用户无需知道该路径）+ 配置集（Lock-only / Bundle 两种格式，§6.7）
- **OS keychain 密钥管理**（`keyring` crate，§9.7）——所有 AI 功能的统一入口
- **AI 合并工作台 AI 辅助模式**（§6.3）
- **AI 增强健康检查**：语义重合度、内容质量、脚本行为分析（§6.5）
- **AI 一键修复**：每条健康问题旁按钮，**强制 diff 预览**（§6.5）
- **语义聚类与颜色高亮**：Voyage embedding API + HDBSCAN（§6.18）
- **Skill 脚手架（内嵌 skill-creator 方法论）**（§6.13）
- **`.agentmix.lock` 版本锁**（§6.9）
- **Source Tracking + 来源仓库更新检测**（§6.8）
- **Git URL 导入**（§6.8 配套）
- **跨工具兼容性预检**（Capability Linter，§6.10）
- **Skill 引用关系检测**（§6.14）
- **RuntimeConflict 显化**（多目标导出后才真正显化，§6.2）
- Skill 编辑器（内置 CodeMirror 实时预览与 frontmatter 校验，§6.6）
- **`scripts/` 安全规则误报白名单**（§6.11）：v0.1 的"每次都强制确认"在长期使用中会让用户麻木点同意，v0.2 引入按 `(source_uri, subpath, rule_id, content_hash)` 粒度的本地白名单，文件内容变化即失效；不随 lock / Bundle 分发

### Phase 3：v0.3——体验扩展与生态打通

把 v0.2 没顾上的"长尾用户场景"覆盖：非技术用户、隐私敏感离线场景、跨机器漫游。

- **反向同步**（Source ← Target，§6.15）
- **加密文件 + 主密码**密钥 fallback——覆盖无 secret service 的 Linux 环境（§9.7）
...
`Asset` 抽象的首次真实回报：扩展到 SKILL 之外的资产类型，验证 §8.2 的抽象设计是否站得住。
...
**决策 6：健康度检查重点关注 description 质量**

description 是 Agent 激活 Skill 的唯一触发依据，其质量直接决定 Skill 是否能被正确使用。过短或描述不清的 description 是 Skill 失效最常见的原因，但现有工具均不对此进行检查。将 description 质量检查作为健康度检查的核心项，是 AgentMix 独特的产品价值之一。

**决策 7：关于产品名称——从 SkillMix 改为 AgentMix**

前期候选名包括 SkillMix（最初定名，强调"组合 Skills"）、SkillWeave、SkillForge。当 Phase 4+ 决定扩展到 slash commands、subagents、CLAUDE.md 片段、MCP 配置等更广义的 Agent 资产后，"Skill"前缀名不副实——会让用户误以为工具只处理 SKILL.md。最终改为 AgentMix，传达"任意 Agent 资产的跨项目组合"。名字与 §8.2 的 `Asset` 抽象、§12 Phase 4 的扩展路线呼应。

**决策 8：MVP 范围收口到 Windows + Claude Code 项目级**

design-doc v0.4 原计划 MVP 一次性覆盖 macOS/Windows/Linux 三平台 + Claude Code/Cursor 双导出。对一人/小团队是过重的范围——每平台都有路径、权限、签名/公证、拖拽行为的长尾问题。MVP 收口到 Windows + Claude Code 项目级单目标，把核心交互（扫描、选择、冲突解决、手动合并、dry-run、scripts/ 预检）跑通，跨平台与多目标导出延后到 v0.2。

**决策 9：Asset 抽象作为架构红线**

跨项目组合的痛点不止存在于 SKILL.md——slash commands、subagents、CLAUDE.md 片段、MCP 配置都有同样的"跨项目组合 + 冲突 + 部署"问题。如果 v0.1 隐含假设"被组合对象 = Skill"，未来扩展时整个 scan/compose/export 流水线都要重写。决策是：v0.1 起就引入 `Asset` 抽象（§8.2），`Skill` 是其唯一具体实现，但代码层禁止直接耦合 `Skill`。这一抽象层 v0.1 必须立起，违反会导致大规模返工。

**决策 10：API 密钥统一通过 OS keychain，不静默降级**

考虑过明文配置文件（直接淘汰，安全性不可接受）、加密文件 + 主密码（UX 摩擦 + KDF 选择长尾）、环境变量（GUI 应用读取不稳定）、云端托管（破"本地开源"定位）后，决定采用 OS keychain（`keyring` crate 跨三平台抽象）作为唯一密钥存储方式。诚实承认其安全边界：能防静态读取与误提交，**不能**防同用户进程读取——但已显著好于明文。Keychain 不可用时，依赖功能禁用并在 UI 明示原因，不允许静默降级到弱存储方案（§9.7）。

**决策 11：备份位置统一在 `~/.agentmix/backups/`**
...
**决策 12：健康检查从字符长度阈值改为结构化校验**

design-doc v0.4 的"description < 20 字符即警告"是机械阈值——21 字符的烂 description 同样会让 Agent 无法激活，而 18 字符的精炼 description 完全可能更好。决策弃用字数判定，改为按内容判断（§6.5）：v0.1 用确定性规则检测是否包含触发场景动词；v0.2 起用 embedding 查同源描述是否语义重复、用 LLM 评估描述是否清晰且聚焦触发场景。字数不再作为质量判定条件（仅保留 spec 的 1024 字符上限作合规校验）。

**决策 13：跨工具兼容性矩阵走社区维护，不追求实时同步**

理想方案是各 AI 工具发布机器可读的能力描述符，AgentMix 实时拉取——但这个标准不存在，等不到。次优是 AgentMix 自己跟踪每个工具的变更——单人/小团队无法实时跟进。决策采用社区 PR 驱动的能力矩阵 `compatibility-matrix.json`（§6.10）：基线随版本发布，启动时远程更新，UI 显示数据新鲜度。坦白告诉用户"基于 X 天前的数据"——比假装"始终准确"更负责。

**决策 14：AI 功能 v0.2 默认走 API（embedding 与 LLM），本地模型延后到 v0.3 opt-in**
...
**决策 15：embedding provider 默认且仅用 Voyage AI，不暴露给用户选择**
...
**决策 16：i18n 在 MVP 即支持中文 + 英文，作为架构红线**
...
**决策 17：自动更新通道——GitHub Releases + Tauri Updater，随 v0.1.5 交付**

桌面应用的首版发布是不可逆事件——首版不带更新机制，用户装完即被困在首版。但在 v0.1 范围进一步收窄到 alpha 后，自动更新被推迟到 v0.1.5 交付（详见决策 19）。本决策仅锁定**通道选型**：`tauri-plugin-updater` + GitHub Releases，开源项目零额外基础设施成本；Release 签名密钥在 CI secret 中管理，公钥嵌入 App；默认开启检查更新（24 小时缓存），用户可在设置中关闭以尊重 air-gapped 场景。v0.1 alpha 阶段通过 GitHub Releases 静态分发，用户手动升级。

**决策 18：AI 一键修复必须强制 diff 预览，永不自动应用**
...
**决策 19：v0.1 定位 alpha，自动更新延到 v0.1.5**
...
**决策 20：i18n 架构红线 vs 翻译完整度解耦——v0.1 ship stub 模式**
...
**决策 21：ExportPlan 作为 Dry-run 红线的数据落地**

Dry-run "preview → confirmation → execution" 红线如果用两条独立代码路径实现（一条算预览数据，一条真正修改文件），极容易出现两者不一致——预览说会写 12 个文件，真改时改了 13 个。决策引入 `ExportPlan` 数据模型（§8.2）：所有破坏性操作产生同一个 `ExportPlan` 对象，Dry-run UI 渲染它、execution 消费它。"一份计划，两阶段使用"，从结构上排除不一致可能。

**决策 22：双冲突模型——ExportConflict vs RuntimeConflict 分开建模**

原设计把"用户在同一组合里勾了两个同名 Skill"和"导出后与目标工具已有 Skill 在运行时争名字"两类冲突压在同一个 name-based 检测里，混淆了文件系统级与运行时解析级两个完全不同的问题。决策拆分（§6.2）：ExportConflict 按"(目标工具, scope, 目标路径, exported_name)"四元组检测，导出前必须解决；RuntimeConflict 按目标工具的 ToolAdapter 行为计算，作为警告级提示展示。这让冲突 UI 能精确告诉用户"为什么冲突、在哪冲突、怎么解决"。

**决策 23：`.agentmix.lock` 不包含 license / description / 其他 SKILL.md 元数据**

GPT 评审建议 lock 中加入 `license` 字段以便合规追踪。决策不加。理由：(1) license 已经在 SKILL.md frontmatter 自身（`license:` 是 spec 字段），lock 重复存储会造成"哪份是权威"的混淆；(2) lock 的职责是**复现**（resolve 来源 → 验证完整性），不是"元数据汇总"——后者属于 SKILL.md 解析层的事；(3) description / metadata 等字段同理，全部不进 lock。Lock schema 严格收敛到 7 字段（§6.9），与 license / metadata 解耦。

**决策 24：安全契约边界——"风险可见"而非"安全保证"**

静态扫描无法证明任何脚本绝对安全，承诺"安全"是过度承诺。决策把 `scripts/` 安全预检的契约改成"风险可见 + 默认保守 + 高危项强制确认"（§6.11）：明确列出能做的（symlink 不跟随、2MB 体积上限、二进制资产清单、高危规则扫描）和明确做不到的（SKILL.md 正文恶意指令、外部文件引用、高级混淆）。给用户**展示**风险，让用户**决策**是否接受，AgentMix **不替**用户判断安全与否——这是契约边界，写明白避免误期待。
...
本附录是 §6.9 的工程细节配套，描述 lock 文件的字段定义、序列化格式、vendor 机制与复现流程。面向工具实现者与需要手动检查 lock 文件的开发者。
...
- 本机用户随时可以 resolve
- §6.7 Bundle 模式导出时，把所有引用的 vendor 副本一并打包到 zip
